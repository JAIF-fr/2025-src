...
|        |[[#nasahl][Pascal Nasahl]]|OpenTitan's Hardware Security Analysis Framework|
|        |[[#casavecchia][Soline Casavecchia]] | Laser Fault Injection Exploration on System-on-Chip |
|        |[[#iooss][Ambre Iooss]] | Passage à l'échelle des campagnes de simulations d’injections de fautes |
| 10h55  |                       | Pause café - [[#posters][posters and demos]]           |
| 11h40  |                       | *Session 2 -- Systems and Countermeasures* |
|        |[[#leveugle][Régis Leveugle]]|Protection contre les fautes : est-ce bien toujours une bonne action pour la sécurité ? |
|        |[[#ferres][Bruno Ferres]]|Using a Vulnerability Assessment Methodology to build and improve Countermeasures against Multi-Fault Injection|
|        |[[#potet][Marie Laure Potet]]|TBD|
| 12h40  |                       | Déjeuner                                 |
| 14h00  |                       | *Session 3 -- Security Evaluation*         |
|        |[[#avoue][Géraldine Avoué]]|Les attaques matérielles dans le cadre de la certification Critères Communs|
|        |[[#charles][Alex Charlès]]|Haystack ciphers : White-box countermeasures as Symmetric encryption|
|        |[[#rais][Idris Raïs-Ali]]|When Is It an Attack? Distinguishing Fault Injection perturbation from Environmental Effects in FPGA-based Digital Sensor|
| 15h10  |                       | Pause café - [[#posters][posters and demos]]           |
| 15h50  |                       | *Session 4 -- Laser and Caracterization*   |
|        |[[#claudepierre][Ludovic Claudepierre]]|Caractérisation des fautes induites par clock-glitch par comparaison avec l’injection laser|
|        |[[#manguin][Loïc Manguin]]|                                          |
|        |[[#douadi][Aghiles Douadi]]|Laser Fault Injection on RO-based PUFs Implemented on FPGA|
...
*Bio*
Alex Charlès conclu, à la fin de l’année, son doctorat à l’Université du Luxembourg sous la supervision d’Alex Biryukov, et a en particulier publié et présenté deux articles scientifiques à la conférence Cryptographic Hardwares and Embedded Systems (CHES) (https://tches.iacr.org/index.php/TCHES/article/view/11168, https://tches.iacr.org/index.php/TCHES/article/view/11668) sur le domaine de la cryptographie en boîte blanche, spécifiquement sur l’étude des schémas de masquages, et possède d’autres travaux en cours de parution sur ce même domaine.
...
| Sébastien Michelland   | [[#michelland][Secure compilation with Tracing LLVM: a demo]]                                                                                    |
| Edna Ferrucho-Alvarez | [[#ferrucho][REVERSE ENGINEERING-BASED FAULT INJECTION IN FPGA BRAM]]                                                                                    |
| Jérôme Hue, Adrian Evans | [[#hue][Utilisation de la plateforme AIDGE pour l’analyse de l’impact des fautes dans des réseaux de neurones]]                                                                                    |
| Mahreen Khan | [[#khan][Gem5-based Virtual Platform for RISC-V Security]]                                                                                    |
| Ziling Liao | [[#liao][A venir]]                                                                                    |
| Yanis Sellami | [[#sellami][A Binsec Plugin for Fault Injection / Adversarial Symbolic Execution]]                                                                                    |
| Daniel Thirion | [[#thirion][ClassifyFP: RTL Fault Classifier for a Low False-Positive Rate Safe & Secure AES]]                                                                                    |
...
*Abstract*.
Most countermeasures against fault injection or side-channel attacks that have software components have to fight their compiler at some point. If the countermeasure is applied early (such as in the source code), it's difficult to prevent the compiler from optimizing away the careful additions. Even when optimizations are not involved, it's often hard (sometimes impossible) to force the use of chosen instructions, registers, or other resources in the target code. If the countermeasure is applied late, most traces of the source code are lost, making it challenging to find all the variables, expressions, and other program elements of interest. Occasionally, a countermeasure needs a bit of both, and then all bets are off.

This adversarial relationship with the compiler doesn't have to be, though. This talk will introduce and showcase Tracing LLVM, an extension to the LLVM compiler designed for writing security countermeasures. Tracing LLVM provides additional control over the compilation process and includes stronger preservation guarantees (at the cost of less optimization). The talk will discuss the main ideas of the project and demo the compiler on small programs.
...
*Abstract*.
Laser fault injection (LFI) is a powerful technique widely used to perform attacks that modify configuration, data, and operation in embedded systems. This method involves pulsed laser illumination that induces a localized disturbance in a transistor, temporarily changing its output state [1]. Performing LFI requires a detailed understanding of the device architecture. In this context, reverse engineering techniques, such as Photo Emission Analysis (PEA), allow the recognition of regions of interest like RAM blocks by capturing photonic emissions from active components in a circuit [2]. Nowadays, BRAM-based FPGAs are extensively used due to their high efficiency, fast data handling capabilities, reconfigurability, and parallelism [3].
...
*Abstract*.
Recent work by Ducousso et. al. [6, 5] has demonstrated that it is possible to design an efficient symbolic execution for binary programs that also takes into account the ability of an attacker to perform fault injections. It was proposed alongside an implementation within the Binsec [4, 1] symbolic execution engine and demonstrated its capabilities to detect attacks on protected software such as the Wookey bootloader [2]. While this implementation is available for research and reproducibility purposes [3], it was not designed to be easily extensible nor to benefit from future advances in the underlying Binsec engine.
...
|------------------+-------------+-------+----------+-----+-----------|
| Glorieux         | IROC        |       |          |     |missing attachment|
| Guichaoua        | IDEMIA/EMSE |       |          |     |missing attachment|
